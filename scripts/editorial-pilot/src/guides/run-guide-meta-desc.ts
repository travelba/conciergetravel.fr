/**
 * CLI — generate `meta_desc_{fr,en}` for `editorial_guides` rows.
 *
 * Mirrors `scripts/editorial-pilot/src/rankings/run-ranking-meta-desc.ts`.
 * Guides reuse the rankings generator + prompt because the SERP card
 * shape is identical (curated editorial selection, not transactional
 * product).
 *
 * Modes:
 *   --slug=<slug>            single guide (debug)
 *   --slugs=a,b,c            explicit list
 *   --limit=<N>              cap to N guides (PILOT mode)
 ...
 *
 * Examples:
 *   pnpm tsx scripts/editorial-pilot/src/guides/run-guide-meta-desc.ts --only-drafts --limit=3 --dry-run
 *   pnpm tsx scripts/editorial-pilot/src/guides/run-guide-meta-desc.ts --only-drafts --concurrency=4
 *
...
} from './supabase-guides.js';
import {
  generateRankingMetaDesc,
  RankingMetaDescError,
  META_DESC_MIN_CHARS,
  META_DESC_MAX_CHARS,
} from '../rankings/meta-desc-generator.js';
...
async function main(): Promise<void> {
  ...
  console.log(`[guide-meta-desc] provider=${provider} model=${client.model}`);
  console.log(
    `[guide-meta-desc] mode dryRun=${args.dryRun} concurrency=${args.concurrency} limit=${args.limit ?? '∞'} includeAll=${args.includeAll} includeDrafts=${args.includeDrafts}`,
  );
  ...
  const allRows = await listGuides(supabase, listOpts);
  ...
  console.log(
    `[guide-meta-desc] fetched=${allRows.length} eligible=${candidates.length} processing=${rows.length}`,
  );
  if (rows.length === 0) {
    console.log('[guide-meta-desc] nothing to do.');
    return;
  }
  ...
  const results = await withConcurrency(
    rows,
    args.concurrency,
    (row) => runOnGuide(client, supabase, row, { dryRun: args.dryRun }),
    (done, total, last) => {
      const status = last.success
        ? `OK (${last.attempts}x, ${last.fr?.length}c/${last.en?.length}c)`
        : `FAIL: ${last.error?.slice(0, 80)}`;
      console.log(
        `[guide-meta-desc] ${done}/${total} ${last.slug} [${last.scope ?? '—'}] → ${status}`,
      );
    },
  );
  ...
  console.log('---');
  console.log(`[guide-meta-desc] DONE in ${(elapsedMs / 1000).toFixed(1)}s`);
  console.log(
    `[guide-meta-desc] success=${okCount} fail=${failCount} tokens=${totalInputTokens} in / ${totalOutputTokens} out`,
  );

  await mkdir(RUNS_DIR, { recursive: true });
  const ts = new Date().toISOString().replace(/[:.]/g, '-');
  const logPath = resolve(RUNS_DIR, `guide-meta-desc-${args.dryRun ? 'dry' : 'live'}-${ts}.json`);
  await writeFile(
    logPath,
    JSON.stringify(
      {
  ...
  console.log(`[guide-meta-desc] run log → ${logPath}`);
...
main().catch((err) => {
  console.error('[guide-meta-desc] FATAL', err);
  process.exit(1);
});
