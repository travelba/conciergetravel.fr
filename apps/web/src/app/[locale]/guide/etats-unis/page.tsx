...
/**
 * `/guide/etats-unis` — first international country guide (Vague 6
...
export async function generateMetadata({
  ...
  const locale = raw;
  const t = await getTranslations({ locale, namespace: 'guideEtatsUnis' });
  const buildCanonicalPath = (l: Locale): string =>
    getPathname({ locale: l, href: '/guide/etats-unis' });
  return {
    title: t('metaTitle'),
...
export default async function GuideEtatsUnisPage({
  params,
}: {
  params: Promise<{ locale: string }>;
  ...
  const url = `${origin}${getPathname({ locale, href: '/guide/etats-unis' })}`;
  ...
  return (
    <main className="container mx-auto max-w-5xl px-4 py-10 sm:py-14">
      ...
      <section
        data-aeo
        aria-labelledby="guideetatsunis-aeo-title"
        className="border-border bg-bg mb-12 rounded-lg border p-5"
      >
        <h2 id="guideetatsunis-aeo-title" className="text-fg font-serif text-lg">
          {t('aeoQuestion')}
        </h2>
      ...
      {/* FAQ */}
      <section aria-labelledby="guideetatsunis-faq-title" className="border-border border-t pt-10">
        <h2 id="guideetatsunis-faq-title" className="text-fg mb-6 font-serif text-2xl sm:text-3xl">
          {t('faqTitle')}
        </h2>
