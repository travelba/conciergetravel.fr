...
> 48 agent skills covering every vertical. Each skill is a `SKILL.md` file
...
| **Phase 9 multi-supplier booking (GIATA + RateHawk + Travelport, opaque pricing — Q43)**                                                                                                                                                                  | [`booking-aggregation-giata`](booking-aggregation-giata/SKILL.md) ⭐ NEW                                                                                          |
| Hotel detail page, room sub-pages, editorial fiches                                                                                                                                                                                                       | [`content-modeling`](content-modeling/SKILL.md)                                                                                                                   |
| **Hotel kit rollout** (pilotes Airelles/PdG → catalogue — F&B, GMB, POI photos, **hero/galerie/chambres Rule 7**, **batch orchestration Rule 8**, ton concierge, gates `kit.*` D15–D22)                                                                   | [`hotel-kit-rollout`](hotel-kit-rollout/SKILL.md) · `kit-fiche-acceptance-gates.ts`                                                                               |
...
| **LLM gate combining hard LENGTH + LEXICON rules** (deterministic clamp/sentence-split for length BEFORE Zod, linter stays sovereign for vocabulary)                                                                                                      | [`llm-output-robustness` §Rule 19](llm-output-robustness/SKILL.md) ✏️                                                                                             |
...
| **FAQ Perplexity catalogue entier** (40–60 factual + 20–30 concierge, obligatoire toutes fiches — rule + skill)                                                                                                                                           | [`hotel-faq-perplexity-enrichment`](hotel-faq-perplexity-enrichment/SKILL.md) + [rule `hotel-faq-perplexity.mdc`](../rules/hotel-faq-perplexity.mdc)              |
...
| **POI → `places` backfill geo-fence** (`hotels.points_of_interest` extraction, `--city-match` stem vs stored label, `--max-radius-km` median-centroid guard against cross-city leaks)                                                                     | [`content-enrichment-pipeline` §Rule 15](content-enrichment-pipeline/SKILL.md) ✏️                                                                                 |
| **Publish-gate discipline for backfill/scaffold pipelines** (a backfill NEVER auto-publishes; one tested gate is the sole publisher exporting `gateFailures`; an idempotent reconciler unpublishes hors-gate rows; module run-guard on `process.argv[1]`) | [`content-enrichment-pipeline` §Rule 16](content-enrichment-pipeline/SKILL.md) ✏️                                                                                 |
| **Keyword/SERP/intent grounding** (DataForSEO → FAQ + titres + GEO/AEO ancrés sur la demande réelle ; PAA select, cluster cache, per-item crash-resilience ; **FAQ kit grounding + gate `dfs_paa_coverage`**)                                             | [`keyword-grounding-dataforseo`](keyword-grounding-dataforseo/SKILL.md) ⭐ + [rule `dataforseo-content-grounding.mdc`](../rules/dataforseo-content-grounding.mdc) |
...
| **Hotel/itinerary photos** (Pinterest hotlink legal risk, Wikimedia → Cloudinary migration, SMD fields, hero fallback)                                                                                                                                    | [`photo-pipeline`](photo-pipeline/SKILL.md) ⭐ NEW                                                                                                                |
| **Lieux (places) photo backfill** (`backfill-place-photos.ts`, OpenAI-free, no `hero_image is null` filter / offset → size `--limit` to the city) + **hotel APPEND path depends on OpenAI Vision**                                                        | [`photo-pipeline` §Lieux backfill + §APPEND depends on OpenAI](photo-pipeline/SKILL.md) ✏️                                                                        |
| **Hotel photos — SEO/GEO/agentique quality contract** (≥30 photos, alt enrichi, Cloudinary preset, JSON-LD `ImageObject`, `getHotelPhotos`)                                                                                                               | [`photo-quality-seo-geo-agentique`](photo-quality-seo-geo-agentique/SKILL.md) ⭐ NEW                                                                              |
| Amadeus Hotels API (search, offers, booking, PNR)                                                                                                                                                                                                         | [`amadeus-gds`](amadeus-gds/SKILL.md) — **v1 legacy; v2 → [`booking-aggregation-giata`](booking-aggregation-giata/SKILL.md)**                                     |
...
| Next.js 16 App Router (routes, metadata, caching, middleware) + **force-dynamic for JSON-LD**                                                                                                                                                             | [`nextjs-app-router`](nextjs-app-router/SKILL.md) ✏️                                                                                                              |
...
| **v2 Booking.com UX parity** (home, SRP, fiche, compte — matrice, mobile-first, PriceSlot, anti-dark-patterns, note /10 UI)                                                                                                                               | [`booking-parity-ux`](booking-parity-ux/SKILL.md) ⭐ NEW                                                                                                          |
...
| **Acceptance must assert the rendered VALUE, not just HTTP 200 / element presence** (counters > 0, expected card count, coherent totals; curl-only = grep the value motif & fail on all-zero — `/classements` "0 hôtels" 2026-06-23)                      | [`user-acceptance-loop` §Assert VALUES](user-acceptance-loop/SKILL.md) ✏️                                                                                         |
| **PostgREST `.in('col', [big id list])` explodes the URL → 414** + the silent-`data`-only trap (scan/`.range()` or chunk ≤100 or aggregate via RPC; ALWAYS destructure & check `error`)                                                                   | [`supabase-postgres-rls` §`.in(...)` URL explosion](supabase-postgres-rls/SKILL.md) ✏️                                                                            |
...
| **PowerShell / Windows dev commands, Supabase SSL strip, `commit -F` for special chars, mystery-commit recovery, `*>` vs `2>&1 \| Tee-Object` fake exit 1, multi-agent pathspec commits**                                                                 | [`windows-dev-environment`](windows-dev-environment/SKILL.md) ✏️                                                                                                  |
| **Concurrent / multi-agent commits — `git commit <pathspec>` never `git add -A` in a shared working tree**                                                                                                                                                | [`commit-conventions.mdc`](../rules/commit-conventions.mdc) + [`windows-dev-environment` §Rule 9 quinquies](windows-dev-environment/SKILL.md) ✏️                  |
...
- [`keyword-grounding-dataforseo`](keyword-grounding-dataforseo/SKILL.md) ⭐ — DataForSEO PAA + related keywords + intent grounding for FAQ/titres/GEO ; cluster cache, per-item crash-resilience, places publish gate.
...
- [`hotel-faq-perplexity-enrichment`](hotel-faq-perplexity-enrichment/SKILL.md) — Perplexity FAQ obligatoire sur toutes les fiches (40–60 kit + 20–30 concierge). Rule : [`hotel-faq-perplexity.mdc`](../rules/hotel-faq-perplexity.mdc).

### Vendor integrations

- [`amadeus-gds`](amadeus-gds/SKILL.md) (v1 legacy), [`booking-aggregation-giata`](booking-aggregation-giata/SKILL.md) ⭐ NEW (Phase 9 / Q43), [`little-hotelier`](little-hotelier/SKILL.md), [`competitive-pricing-comparison`](competitive-pricing-comparison/SKILL.md), [`search-engineering`](search-engineering/SKILL.md), [`payment-orchestration`](payment-orchestration/SKILL.md), [`email-workflow-automation`](email-workflow-automation/SKILL.md).
...
- [`booking-engine`](booking-engine/SKILL.md), [`booking-aggregation-giata`](booking-aggregation-giata/SKILL.md) ⭐ NEW, [`loyalty-program`](loyalty-program/SKILL.md), [`membership-program`](membership-program/SKILL.md), [`whatsapp-concierge-journey`](whatsapp-concierge-journey/SKILL.md) ⭐ NEW — accompagnement concierge proactif de chaque client ayant réservé (l'ambition cœur du site).
...
| **Hotel photo audit + Pinterest hotlink risk** (214 illegal hotlinks detected 2026-05-25)                                                                                                                                  | `photo-pipeline` ⭐                                                                          |
| Itinerary hero **fallback** to first-hotel `hero_image` (unblocks 5/20 without data migration)                                                                                                                             | `photo-pipeline` §fallback                                                                   |
| Wikimedia → Cloudinary bulk migration pipeline                                                                                                                                                                             | `photo-pipeline` §Migration priorities                                                       |
| Cloudinary **Structured Metadata fields** for hotel photos (hotel_slug, photo_category, alt_text_fr/en, credit, licence)                                                                                                   | `photo-pipeline` §canonical-shape                                                            |
...
| **Audit metric mirrors the production Zod envelope, not the CDC ideal** (factual_summary "239 gap" → really 1, 2026-05-25)                                                                                                 | `llm-output-robustness` rule 14 + `AGENTS.md` §4bis                                          |
...
| **Deterministic length-salvage before Zod/gate** (clamp summaries pre-parse + split sentences > 25 mots at natural boundaries; lexicon stays LLM/linter-governed) — generalises the rankings `justification ≤ 1200c` clamp | `llm-output-robustness` §Rule 19 ✏️ (ref: `places/enrich-places-editorial.ts`)               |
| **Geo-fence on multi-city POI backfill** (fragmented `hotels.city` + cross-city POI leaks → median-centroid radius guard; London 42→34 scaffolds, all `is_published=false`)                                                | `content-enrichment-pipeline` §Rule 15 ✏️ (ref: `places/backfill-paris.ts`)                  |

## 7. June 2026 — capitalisation log

| Pattern paid for                                                                                                                                                                                                                                                                                                | Capture                                                                                                               |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| **`2>&1 \| Tee-Object` raises a spurious `NativeCommandError` → fake exit 1** on a successful native command; use the `*>` PowerShell redirect (2026-06-21)                                                                                                                                                     | `windows-dev-environment` §Rule 6b ✏️                                                                                 |
| **Multi-agent / background subagents share one Git index** → `git add -A` cross-contaminates; commit with an explicit pathspec + 3s `index.lock` retry (2026-06-21)                                                                                                                                             | `windows-dev-environment` §Rule 9 quinquies ✏️ + `commit-conventions.mdc` §Concurrent / multi-agent commits           |
| **Backfill/scaffold publish-gate discipline** — a backfill auto-published 199 thin Paris stubs live; fix = default `is_published=false`, one exported gate as sole publisher, idempotent reconciler (229→102), `process.argv[1]` run-guard (a dry-run import published 72 rows) — commit `08bb0f2` (2026-06-21) | `content-enrichment-pipeline` §Rule 16 ✏️ (ref: `places/{backfill-paris,publish-places,reconcile-places-publish}.ts`) |
| **Lieux (places) photo backfill `backfill-place-photos.ts` has NO `hero_image is null` filter and NO offset** → small `--limit` re-scans the same first-N alphabetical slugs forever; size `--limit` to the whole city in one pass (2026-06-22)                                                                 | `photo-pipeline` §Lieux backfill ✏️ (ref: `places/backfill-place-photos.ts`)                                          |
| **No hotel photo APPEND path without OpenAI** — `upload-press-kit-images.ts` calls `visionAnalyse()` unconditionally (alt/category/keep); the only OpenAI-free Places path `sync-hotel-photos --tier=places` OVERWRITES the gallery; real `<10`-photo residual ~15 not ~87 (2026-06-22)                         | `photo-pipeline` §APPEND depends on OpenAI Vision ✏️                                                                  |
| **Wikidata NEW match must be corroborated (geo OR accommodation description)** — coordinate-less false positives (taxa/films/people) bypass the geo guard (`Rosa Alpina` → plant species `Q87641905`) — commit `fd451b75` (2026-06-22)                                                                          | `content-enrichment-pipeline` §Rule 9 bis ✏️                                                                          |
| **New squatter family `hotels<geo><digits>` glued non-www subdomain** — trailing digits safely re-admit `.com` without catching `langhamhotels.com` — commit `fd451b75` (2026-06-22)                                                                                                                            | `photo-pipeline` §New squatter family ✏️ (ref: `enrichment/toxic-official-url.ts`)                                    |
| **A too-short city-match stem contaminates HOMONYM cities** (`Floren` captures Saint-Florent, skews the geo-fence centroid) — use the exact spelling (2026-06-22)                                                                                                                                               | `content-enrichment-pipeline` §Rule 15 ✏️                                                                             |
| **PostgREST `.in(688 UUIDs)` explodes the URL → 414, swallowed as silent `null` → `/classements` showed "0 hôtels" on every card** — scan/`.range()` (or chunk ≤100 / aggregate via RPC) + ALWAYS check `error`; same "PostgREST scaling" family as the 1000-row cap — commit `53eb7e8` (2026-06-23)            | `supabase-postgres-rls` §`.in(...)` URL explosion ✏️ + `content-enrichment-pipeline` §1000-row gotcha (sister)        |
| **Acceptance must assert the rendered VALUE, not just HTTP 200 / element presence** — the Go-Live re-audit passed the "0 hôtels" page because it checked status + markers, not values; curl-only = grep `\b([1-9]\d*)\s+hôtels\b` and fail on all-zero (2026-06-23)                                             | `user-acceptance-loop` §Assert VALUES ✏️ (ref: `apps/web/src/server/rankings/get-ranking-by-slug.ts`)                 |
