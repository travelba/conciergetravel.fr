...
**Verdict (mai 2026, post-session 4)** : 6 blocages sur 8 résolus. Le **code applicatif** est désormais V2-ready (`SupportedLocale = 'fr' | 'en' | 'de' | 'es' | 'it'` exhaustif, helpers `KnownLocale` couvrant V3, plus aucun ternaire `locale === ...` applicatif, **`routing.pathnames` complet avec placeholders DE/ES/IT en commentaire prêts à activer**). Les 2 blocages restants — **#5 schéma DB dual-locale** et **#6 messages `de/es/it.json`** — bloquent encore l'activation d'une nouvelle locale ; ils sont adressés en Phase 3 et 4 respectivement. Activer naïvement `'de'` dans `routing.locales` aujourd'hui produirait toujours `/de/...` qui rendraient du contenu français (les readers servent un FR fallback documenté en attendant la Phase 3) avec `<html lang="de">` — cloaking SEO involontaire, signal catastrophique pour Google.

**Conséquence concrète de la Phase 2** : ajouter une locale à `routing.locales` activera **automatiquement** la résolution des slugs localisés (`/de/suche`, `/es/buscar`, `/it/cerca`, `/de/buchung/start`, …) pour les UI / system routes, sans modifier un seul call-site. Les `<Link>`, `getPathname`, `redirect` et les sitemaps lisent désormais tous le `pathnames` typé.

**Les 8 blocages structurels** (6 ✅ résolus, 2 ⏳ restants) :

| #   | Blocage                                                                                                         | Fichier / preuve                                                                                                                                                                            | Impact si ignoré                                                                |
| --- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| 1   | ✅ Résolu (session 4) — `routing.pathnames` cartographie tous les UI / system routes avec placeholders V2 prêts | `apps/web/src/i18n/routing.ts` ; `<Link>` / `getPathname` / `redirect` migrés sur tout `apps/web/src/app/**` ; middleware émet 301 sur les legacy slugs FR sous `/en/`                      | (résolu) Ajouter `'de'` à `routing.locales` activera instantanément `/de/suche` |
| 2   | ✅ Résolu (session 3) — `SupportedLocale = 'fr' \| 'en' \| 'de' \| 'es' \| 'it'`                                | `apps/web/src/i18n/supported-locale.ts` ; consommé par tous les readers `pickXxx` / `readXxx` via `pickLocalizedText` + `pickByLocale` + `assertNever`                                      | (résolu) Type-level exhaustivité garantie ; FR fallback documenté pour DE/ES/IT |
| 3   | ✅ Résolu (sessions 2 + 3) — plus aucun ternaire applicatif                                                     | Audit `rg` ne renvoie plus que JSDoc + fixtures de test + `isV1Locale` + 3 callbacks `buildHreflangAlternates` + `locale-switcher` V1-binaire — tous intentionnels                          | (résolu) PR review refuse tout nouveau ternaire `locale === ...`                |
| 4   | ✅ Résolu (session 2) — `withLocalePrefix` collapsé sur `withLocalePath`                                        | Helpers centralisés dans `apps/web/src/i18n/runtime.ts` (élargis à `KnownLocale` V3-ready) ; `buildHreflangAlternates` itère `routing.locales`                                              | (résolu) Ajout d'une locale = 0 ternaire à éditer                               |
...
| 8   | ✅ Résolu (session 2) — `intlLocaleTag(locale)` / `ogLocale(locale)` centralisés                                | Helpers dans `apps/web/src/i18n/runtime.ts` couvrent les 8 locales planifiées via `Record<KnownLocale, …>` exhaustifs                                                                       | (résolu) Dates + OG corrects sur ajout de locale, vérifié par tests vitest      |
...
2. **Phase 1 — Refactor type-safe (1-2 jours, sans contenu)** :
   ...
   - ✅ **Sous-étape 1c-α livrée** (session 3) : `SupportedLocale = 'fr' | 'en' | 'de' | 'es' | 'it'` exporté depuis [`apps/web/src/i18n/supported-locale.ts`](../../../apps/web/src/i18n/supported-locale.ts), accompagné de :
     - `assertNever` helper dans [`apps/web/src/lib/assert-never.ts`](../../../apps/web/src/lib/assert-never.ts).
     - `pickLocalizedText(locale, fr, en)` — picker centralisé avec policy V2 (FR/DE/ES/IT → `fr ?? en`, EN → `en ?? fr`). C'est le **seul point** à modifier le jour où les `*_translations` DE/ES/IT seront servies, en Phase 3.
     - `pickByLocale(locale, frBranch, enBranch)` — variante pour les branches non-textuelles (slugs SQL, candidate keys, colonnes).
     - Tous les readers `pickXxx` data-layer (`server/destinations/cities.ts`, `server/hotels/get-{hotel,room}-by-slug.ts`, `server/hotels/dev-fake-hotel-detail.ts`, plus les routes `app/[locale]/**` avec ternaires data-layer) ré-écrits en `switch (locale) { case 'fr': ...; case 'de': case 'es': case 'it': /* FR fallback documenté */; case 'en': ...; default: assertNever(locale); }`.
   - ✅ **Sous-étape 1c-β livrée** (session 3) : toutes les copy maps `{ fr: '...', en: '...' }` migrées vers `next-intl` (`getTranslations` pour les server components, `useTranslations` pour les client). Surfaces couvertes :
     - Composants éditoriaux : `last-updated-badge`, `toc-sidebar`, `editorial-glossary`, `editorial-callout`, `editorial-table`, `external-sources-footer`.
     - Composants hôtel / rankings : `hotel-tldr`, `hotel-featured-in-rankings`, `related-hotels`, `rankings-facets` (la prop `locale` supprimée — le composant lit le contexte `next-intl` directement).
     - Email subjects : `server/booking/{confirm-payment,email-request}.ts` migrent vers la namespace `emails`.
   - **Bénéfice acquis** : un `rg "locale === ['\"]fr['\"]"` sur `apps/web/src/` ne renvoie plus que des occurrences intentionnelles (JSDoc des helpers, fixtures de test, `isV1Locale`, callbacks `buildHreflangAlternates` qui sélectionnent un slug, et le `locale-switcher` V1-binaire). Aucun ternaire applicatif à migrer. **Phase 1c close.** Toute nouvelle PR doit utiliser `pickLocalizedText` / `pickByLocale` / `next-intl` messages — un revue refuse tout nouveau ternaire `locale === ...`.

3. **Phase 2 — `routing.pathnames`** : ✅ livré (session 4, commits `4419aea` → `5587056` + `e930a1c`). Mapping complet dans [`apps/web/src/i18n/routing.ts`](../../../apps/web/src/i18n/routing.ts) avec placeholders V2 (DE/ES/IT) commentés ; tous les `<Link>`, `getPathname`, `redirect`, sitemaps + canonical/hreflang routés à travers les `pathnames` typés ; le middleware émet un 301 permanent pour les URL legacy `/en/<fr-slug>` vers le slug EN localisé (table dans [`apps/web/src/i18n/legacy-en-redirects.ts`](../../../apps/web/src/i18n/legacy-en-redirects.ts), 14 tests vitest dans `middleware.test.ts`). **Bénéfice acquis** : ajouter une locale à `routing.locales` (Phase 4) activera automatiquement la résolution des slugs localisés — zéro call-site à éditer.
