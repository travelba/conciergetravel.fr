...
| --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| POI `image_public_id` = slot galerie `press-*` (chambre sur carte Musée YSL)                                                | UX trompeuse, fail gate `gold.poi_photo_structural`                                                            | `resource-{slug}-poi-images.ts` → `poi-{slug}` · skill `photo-pipeline` §Photo-subject correspondence |
| `category=spa` mais `alt_fr` décrit une chambre                                                                             | Spa block affiche mauvais sujet                                                                                | Re-sourcer + `categorize-with-vision.ts` · gate `photos.gallery_alt_category`                         |
