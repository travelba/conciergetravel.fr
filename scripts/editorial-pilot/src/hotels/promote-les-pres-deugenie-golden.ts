/**
 * CLI — promote Les Prés d'Eugénie "golden template" editorial content into
 * the real `public.hotels` row.
 *
 * The golden content (restaurants, POI, La Ferme Thermale, FAQ, concierge blocks,
 * Instagram teaser, gallery, MICE, …) is the single source of truth in
 * `@mch/domain/editorial` (`les-pres-deugenie-golden.ts`), shared with the apps/web
 ...
 *   --slug=<slug>    target slug (default: les-pres-deugenie)
 *   --print-json     also dump the full payload that would be written
 *
 * Examples:
 *   pnpm tsx scripts/editorial-pilot/src/hotels/promote-les-pres-deugenie-golden.ts --dry-run
 *   pnpm tsx scripts/editorial-pilot/src/hotels/promote-les-pres-deugenie-golden.ts
 */
...
import {
  LES_PRES_DEUGENIE_PROMOTE_SLUG,
  buildLesPresDeugenieGoldenFields,
  type LesPresDeugenieGoldenInput,
...
interface PromoteRow extends LesPresDeugenieGoldenInput {
  readonly id: string;
...
function parseArgs(argv: readonly string[]): {
  ...
  return {
    slug:
      typeof slugRaw === 'string' && slugRaw.length > 0 ? slugRaw : LES_PRES_DEUGENIE_PROMOTE_SLUG,
...
async function fetchPromoteRow(cfg: SupabaseRestConfig, slug: string): Promise<PromoteRow | null> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `[promote-les-pres-deugenie] SELECT failed (${res.status}): ${body.slice(0, 300)}`,
    );
  }
  ...
  return candidate as PromoteRow;
}
...
async function patchRow(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `[promote-les-pres-deugenie] PATCH failed (${res.status}): ${responseBody.slice(0, 300)}`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`[promote-les-pres-deugenie] slug=${args.slug} dryRun=${args.dryRun}`);

  const row = await fetchPromoteRow(cfg, args.slug);
  if (row === null) {
    console.error(`[promote-les-pres-deugenie] no hotel row found for slug "${args.slug}".`);
    process.exit(1);
  }
  console.log(`[promote-les-pres-deugenie] target row: ${row.name} (${row.id})`);

  const fields = buildLesPresDeugenieGoldenFields({
  ...
  console.log('[promote-les-pres-deugenie] fields to write:');
  for (const [key, value] of Object.entries(fields)) {
    console.log(`  - ${key}: ${describeValue(value)}`);
  }

  if (args.printJson) {
    console.log('[promote-les-pres-deugenie] payload JSON:');
    console.log(JSON.stringify(fields, null, 2));
  }

  if (args.dryRun) {
    console.log('[promote-les-pres-deugenie] DRY RUN — no write performed.');
    return;
  }

  await patchRow(cfg, row.id, fields);
  console.log(
    `[promote-les-pres-deugenie] ✅ wrote ${Object.keys(fields).length} fields to ${row.slug}.`,
  );
}

main().catch((err) => {
  console.error('[promote-les-pres-deugenie] FATAL', err);
  process.exit(1);
});
