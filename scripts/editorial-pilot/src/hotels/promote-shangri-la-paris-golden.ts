/**
 * CLI — promote the Shangri-La Paris "golden template" editorial content into
 * the real `public.hotels` row.
 *
 * Golden content lives in `@mch/domain/editorial` (`shangri-la-paris-golden.ts`),
 ...
 *   --slug=<slug>    target slug (default: shangri-la-paris)
 *   --print-json     also dump the full payload that would be written
 *
 * Examples:
 *   pnpm --filter @mch/editorial-pilot promote:shangri-la-paris-golden:dry
 *   pnpm --filter @mch/editorial-pilot promote:shangri-la-paris-golden
 */
...
import {
  SHANGRI_LA_PARIS_PROMOTE_SLUG,
  buildShangriLaParisGoldenFields,
  type ShangriLaParisGoldenInput,
...
interface PromoteRow extends ShangriLaParisGoldenInput {
  readonly id: string;
...
function parseArgs(argv: readonly string[]): {
  ...
  return {
    slug:
      typeof slugRaw === 'string' && slugRaw.length > 0 ? slugRaw : SHANGRI_LA_PARIS_PROMOTE_SLUG,
...
async function fetchPromoteRow(cfg: SupabaseRestConfig, slug: string): Promise<PromoteRow | null> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `[promote-shangri-la-paris] SELECT failed (${res.status}): ${body.slice(0, 300)}`,
    );
  }
...
async function patchRow(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `[promote-shangri-la-paris] PATCH failed (${res.status}): ${responseBody.slice(0, 300)}`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`[promote-shangri-la-paris] slug=${args.slug} dryRun=${args.dryRun}`);

  const row = await fetchPromoteRow(cfg, args.slug);
  if (row === null) {
    console.error(`[promote-shangri-la-paris] no hotel row found for slug "${args.slug}".`);
    process.exit(1);
  }
  console.log(`[promote-shangri-la-paris] target row: ${row.name} (${row.id})`);

  const fields = buildShangriLaParisGoldenFields({
  ...
  console.log('[promote-shangri-la-paris] fields to write:');
  for (const [key, value] of Object.entries(fields)) {
    console.log(`  - ${key}: ${describeValue(value)}`);
  }

  if (args.printJson) {
    console.log('[promote-shangri-la-paris] payload JSON:');
    console.log(JSON.stringify(fields, null, 2));
  }

  if (args.dryRun) {
    console.log('[promote-shangri-la-paris] DRY RUN — no write performed.');
    return;
  }

  await patchRow(cfg, row.id, fields);
  console.log(
    `[promote-shangri-la-paris] ✅ wrote ${Object.keys(fields).length} fields to ${row.slug}.`,
  );
}

main().catch((err) => {
  console.error('[promote-shangri-la-paris] FATAL', err);
  process.exit(1);
});
