...
| **Refonte v2 Booking-like** (`apps/web-v2`, `@mch/ui-v2`, schéma SQL `v2`, BFF `/api/agent` + MCP)                                                          | [`docs/cdc/cahier-des-charges-v2-booking-like.md`](docs/cdc/cahier-des-charges-v2-booking-like.md) + [`.cursor/skills/booking-parity-ux/SKILL.md`](.cursor/skills/booking-parity-ux/SKILL.md) + [`.cursor/rules/v2-migration-freeze.mdc`](.cursor/rules/v2-migration-freeze.mdc) + ADR [0032](docs/adr/0032-v2-booking-parity-ui-rating.md) / [0033](docs/adr/0033-v2-sql-schema-isolation.md) |
| **Booking aggregation GIATA** (multi-fournisseurs Phase 9 — RateHawk, Travelport, Expedia, Bedsonline)                                                      | [`.cursor/skills/booking-aggregation-giata/SKILL.md`](.cursor/skills/booking-aggregation-giata/SKILL.md) + `packages/integrations/src/booking-aggregation/` + [ADR-0026 multi-supplier](docs/adr/0026-multi-supplier-booking-giata.md)                                                                                                                                                         |
...
8ter. **Toute création de contenu / fiche doit être ancrée et vérifiée par DataForSEO** (décision PO 2026-06-25). Aucun texte généré (FAQ kit/concierge, `description_*`, `long_description_sections`, `meta_desc_*`, `factual_summary_*`, GEO/AEO, justifications de classement) n'est persisté sans (a) **grounding DataForSEO** chargé avant génération via `groundHotel(...)` (cache `data/dfs-cache/`, mots-clés / volume / intent / People-Also-Ask injectés dans le prompt) et (b) un **gate de couverture PAA/intent** post-génération en plus de `hasLeak()`. Dégradation propre si DataForSEO est indisponible : logguer `grounding=off`, marquer non-grounded, ne jamais publier en se faisant passer pour ancré. See [`.cursor/rules/dataforseo-content-grounding.mdc`](.cursor/rules/dataforseo-content-grounding.mdc) (hard rule, always-apply) and [`.cursor/skills/keyword-grounding-dataforseo/SKILL.md`](.cursor/skills/keyword-grounding-dataforseo/SKILL.md).
...
Catalogue snapshot (refreshed 2026-06-01 — post matrix re-scan + bulk ranking generation + chain rankings wave 2 + 3 multi-country itineraries):

| Surface              | Total | Published | Draft |
| -------------------- | ----- | --------- | ----- |
| `hotels`             | 2219  | **2219**  | 0     |
| `editorial_rankings` | 576   | **563**   | 13    |
...
drafts (85). Itineraries are 23/23 published — done (incl. 3 multi-country added 2026-06-01).
...
**3rd wave — external_sources coverage + toxic cleanup (2026-06-02)**

The Phase 1.5 `external_sources` backfill was completed and de-polluted:

- **Coverage 61.8 % → 96.2 %** (`external_sources` populated jumped from
  1370 → **2134 / 2219** published). Only 85 published hotels still carry
  no provenance (rows with zero resolvable identifier).
- **827 hotels** now carry a `source: 'official_site'` entry (honest
  attribution). The convertor previously hard-labelled every
  `official_url` as `source: 'wikidata'` (assuming a P856 resolution) —
  false for the scaffold cohort that has `wikidata_id IS NULL`. Patched:
  `official_url` → `wikidata`/high only when the row has a `wikidata_id`,
  else `official_site`/medium. The footer reader maps by `field` so the
  rendered "Official website" link is unchanged; only the LLM-served
  attribution (`/api/agent/hotel-sources`) is now correct.
- **140 toxic `official_url` NULLed** + **110 `external_sources` entries
  stripped** (SEO-squatter `*.com-hotel.com` / `*.{cc}-{city}.info`,
  `h-rez.com` booking engines, `tripadvisor.in` / `us.trip.com` OTA).
  A defensive `isToxicOfficialUrl()` guard now lives in the convertor so
  the spam never re-enters on a future run. **0 toxic remaining.** Full
  detection criteria + the 140-row list:
  `docs/audits/toxic-official-url-cleanup-2026-06-02.md`.
  ⚠ The first detection regex matched `hotels\.com` as a substring and
  nearly NULLed dozens of legit brand domains (`rosewoodhotels.com`,
  `bulgarihotels.com`, `comohotels.com`, `tajhotels.com`, …) — OTA names
  MUST be anchored to the registrable domain (`://([a-z0-9-]+\.)*hotels\.com`).
- Next gap: the 140 cleaned hotels now have no official site at all —
...
- 43 FR / 60 EN `factual_summary` rows remain in the production
  ...
  in-envelope attempt. These are the structurally thin-source hotels
  (Belmond, Ritz-Carlton APAC, citizenM, Greek resorts) where the
  source text doesn't give the generator enough hooks. Defer to a
  manual editorial sweep — ROI on more LLM passes is ~zero.
- 1 hotel (`andbeyond-phinda-private-game-reserve-lodges`) keeps a
  `_synthetic: true` `policies` payload. It is a private game reserve
  with multiple all-inclusive lodges where check-in/out is agent-
  mediated and per-lodge; the safe defaults (15:00 / 12:00, wifi
  whole-property) render fine. Fabricating specific policies from a
  web search would be inaccurate for an IATA OTA, and Tavily tooling
  is offline locally. Accepted as a 1/2219 edge case (0.045 %) flagged
  for the future Google Places / Tavily enrichment pass (Phase 2+).
...
| **2 — Photo pipeline**                              | 🟡 partial 2026-05-31                | Sourcing, legal hygiene, Cloudinary migration, alt enrichment, Structured Metadata fields, hero fallback chain, JSON-LD `ImageObject` (≥ 30 photos per hotel, 10 categories). See `.cursor/skills/photo-pipeline/SKILL.md`. **Alt+category+caption status**: 22 110 photos catalogued, **22 083 / 22 110 (99.88 %)** carry `alt_fr`+`alt_en`+`category`+`caption_fr`+`caption_en` after the 2026-05-31 evening Vision run (`categorize-with-vision` — 43.7 min, $1.94, 2774 photos classified, 195 dropped). **Representativeness + hero_suitable scores**: 21 709 / 22 110 (98.19 %). **Remaining gaps**: (a) the 10-category coverage floor (CDC §2.2) is still 0/2219 — each hotel averages ~10 photos covering 4.7 distinct categories (need Phase 2 sourcing of additional spa/pool/events/concierge photos); (b) ✅ **width/height now 23 145 / 23 145 (100 %)** — backfilled 2026-06-02 from the Cloudinary Admin API via the new `scripts/editorial-pilot/src/photos/backfill-dimensions.ts` pipeline (+ reusable `listUploadedDimensions` helper in `@mch/integrations/cloudinary`). Note: the hotel-detail JSON-LD `ImageObject` was already Hard-Rule-16 compliant — it emits transform-matched dims (hero 1600×900, tiles 1230×820 in `c_fill`) tied to the delivered URL; the backfill closes the _data-completeness_ gap (intrinsic dims now available to any future consumer / anti-CLS rendering); (c) 1916 / 2219 published hotels have ≥ 10 photos (86 %), 87 hotels < 10 photos = the Tier C residual to chase.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **3 — Editorial pages (guides + rankings + itin.)** | 🟡 partial 2026-05-28                | 14 city guides ALL generated + published in `editorial_guides` (NYC, Dubaï, Bali, Tokyo, Amalfi Coast, Marrakech, Mykonos, Santorin, St-Moritz, Phuket, Lake Como, Madeira, Riviera Maya, Algarve — 12-14 sections, 29-42 FAQ, 16-18 TOC anchors, 5-10 sources each). **ADR-0015 step 1 — ✅ DONE (re-verified 2026-06-01)**: `/destination/[citySlug]/page.tsx` now inlines the long-read guide after the hotel hub, in a 2-col layout with a sticky "ON THIS PAGE" TOC + composite `Article` JSON-LD (`@id #guide-article`, `isPartOf #place`). Prod walk confirms it on marrakech/paris (FR) + tokyo (EN). The blocker described in the 2026-05-28 audit was already resolved — the doc was stale. **Region/cluster guides — ✅ SURFACED (2026-06-01, commit `40d1632`)**: the 21 `cluster` (provence, cote-d-azur, luberon, alpes, pays-basque…) + `region` (alsace, bretagne, occitanie, normandie…) `editorial_guides` rows (11-12 bilingual sections + TOC + 20-25 FAQ each) now render as standalone long-reads at `/destination/<slug>` via `<StandaloneGuidePage>` (`apps/web/src/components/destination/standalone-guide-page.tsx`) — a guide-only fallback in `destination/[citySlug]/page.tsx` when no city hub exists. Discoverable in 1 click from the new "France — par région & terroir" section on `/destination`; emitted in `hubs.xml`. JSON-LD: `Article` + `AdministrativeArea` + `BreadcrumbList` + `FAQPage`. Walked on preview FR+EN desktop+mobile. **Country guide gap (deferred)**: 45 `country`-scope rows exist but split into two formats — 10 "clean" slugs (espagne, italie, japon…) carry `sections`+`toc`+EN and are renderable; 35 `guide-*` slugs (guide-portugal, guide-maroc…) carry only `editorial_sections` and are **FR-only (0 EN bodies)**. Surfacing them needs (a) an LLM EN pass on ~315 sections + (b) slug dedup (9 clean/guide-\* twins) + (c) reconciliation with the 8 hand-built `/guide/<country>` i18n pages that stay canonical. Tracked as commit-2 follow-up. See [ADR-0015 §Implementation status](docs/adr/0015-merge-guide-destination.md). Rankings: **22 chain rankings live in prod** — the original 8 (Aman, Four Seasons, Mandarin Oriental, Six Senses, Cheval Blanc, Belmond, Rosewood, Park Hyatt) + **wave 2 (14 published 2026-06-01)**: Ritz-Carlton, St. Regis, Anantara, Waldorf Astoria, Jumeirah, Raffles, Bulgari, Peninsula, Capella, Auberge Resorts, Shangri-La, One&Only, Oberoi, Soneva — via the self-contained batch runner `scripts/editorial-pilot/src/rankings/run-chain-rankings-batch.ts` (filters `out/hotels-catalog.json` by brand regex, generates + pushes directly). Itineraries: **23/23 published** — the original 20 + **3 multi-country (published 2026-06-01)**: `grand-tour-aman-asie` (JP·TH·ID·BT, 5 Aman, 16 j), `mediterranee-belmond` (IT·ES, 5 Belmond, 15 j), `maldives-atoll-hopping` (MV, 4 atolls, 13 j) — via the new LLM runner `scripts/editorial-pilot/src/itineraries/run-itinerary-llm.ts` (OpenAI prose + **live DB hotel + ranking resolution** + direct push, 3-attempt validation loop). All 6 walked on prod FR+EN. |
