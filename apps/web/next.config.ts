...
import { HOTEL_DEDUPE_REDIRECTS } from './src/lib/seo/hotel-dedupe-redirects';
...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    ...
    return [
      ...
      // Twin-fiche dedupe (2026-06-29): 55 duplicate hotel rows were unpublished
      // and their editorial-ranking entries migrated to the canonical (richest
      // live) fiche. Each dropped slug 301s to the canonical. FR is served bare
      // (`localePrefix: 'as-needed'`), EN under `/en` (resolved via `slug_en`).
      // Generated by scripts/editorial-pilot/src/rankings/tmp-dedupe-hotels.ts.
      ...HOTEL_DEDUPE_REDIRECTS.flatMap((r) => [
        { source: `/hotel/${r.fromFr}`, destination: `/hotel/${r.toFr}`, permanent: true },
        { source: `/en/hotel/${r.fromEn}`, destination: `/en/hotel/${r.toEn}`, permanent: true },
      ]),
    ];
  },
};
