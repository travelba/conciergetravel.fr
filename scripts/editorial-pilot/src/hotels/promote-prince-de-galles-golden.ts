/**
 * CLI — promote the Prince de Galles Paris "golden template" editorial content into
 * the real `public.hotels` row.
 *
 * The golden content (restaurants, POI, CALMA Wellness Suite, FAQ, concierge blocks,
 * Instagram teaser, gallery, MICE, …) is the single source of truth in
 * `@mch/domain/editorial` (`prince-de-galles-golden.ts`), shared with the apps/web
 ...
 *   --slug=<slug>    target slug (default: prince-de-galles-paris)
 *   --print-json     also dump the full payload that would be written
 *
 * Examples:
 *   pnpm tsx scripts/editorial-pilot/src/hotels/promote-prince-de-galles-golden.ts --dry-run
 *   pnpm tsx scripts/editorial-pilot/src/hotels/promote-prince-de-galles-golden.ts
 *
...
import {
  ...
  type PrinceDeGallesGoldenInput,
...
interface PromoteRow extends PrinceDeGallesGoldenInput {
  readonly id: string;
...
function parseArgs(argv: readonly string[]): {
  ...
  return {
    slug:
      typeof slugRaw === 'string' && slugRaw.length > 0 ? slugRaw : PRINCE_DE_GALLES_PROMOTE_SLUG,
...
async function fetchPromoteRow(cfg: SupabaseRestConfig, slug: string): Promise<PromoteRow | null> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `[promote-prince-de-galles] SELECT failed (${res.status}): ${body.slice(0, 300)}`,
    );
  }
...
async function patchRow(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `[promote-prince-de-galles] PATCH failed (${res.status}): ${responseBody.slice(0, 300)}`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`[promote-prince-de-galles] slug=${args.slug} dryRun=${args.dryRun}`);

  const row = await fetchPromoteRow(cfg, args.slug);
  if (row === null) {
    console.error(`[promote-prince-de-galles] no hotel row found for slug "${args.slug}".`);
    process.exit(1);
  }
  console.log(`[promote-prince-de-galles] target row: ${row.name} (${row.id})`);

  const fields = buildPrinceDeGallesGoldenFields({
  ...
  console.log('[promote-prince-de-galles] fields to write:');
  for (const [key, value] of Object.entries(fields)) {
    console.log(`  - ${key}: ${describeValue(value)}`);
  }

  if (args.printJson) {
    console.log('[promote-prince-de-galles] payload JSON:');
    console.log(JSON.stringify(fields, null, 2));
  }

  if (args.dryRun) {
    console.log('[promote-prince-de-galles] DRY RUN — no write performed.');
    return;
  }

  await patchRow(cfg, row.id, fields);
  console.log(
    `[promote-prince-de-galles] ✅ wrote ${Object.keys(fields).length} fields to ${row.slug}.`,
  );
}

main().catch((err) => {
  console.error('[promote-prince-de-galles] FATAL', err);
  process.exit(1);
});
