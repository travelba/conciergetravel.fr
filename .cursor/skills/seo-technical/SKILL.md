...
**Verdict (mai 2026, post-session 3)** : 5 blocages sur 8 résolus. Le **code applicatif** est désormais V2-ready (`SupportedLocale = 'fr' | 'en' | 'de' | 'es' | 'it'` exhaustif, helpers `KnownLocale` couvrant V3, plus aucun ternaire `locale === ...` applicatif). Les 3 blocages restants — **#1 `routing.pathnames`**, **#5 schéma DB dual-locale**, **#6 messages `de/es/it.json`** — bloquent encore l'activation d'une nouvelle locale ; ils sont adressés en Phase 2, 3 et 4 respectivement. Activer naïvement `'de'` dans `routing.locales` aujourd'hui produirait toujours `/de/...` qui rendraient du contenu français (les readers servent un FR fallback documenté en attendant la Phase 3) avec `<html lang="de">` — cloaking SEO involontaire, signal catastrophique pour Google.

**Les 8 blocages structurels** (5 ✅ résolus, 3 ⏳ restants) :
