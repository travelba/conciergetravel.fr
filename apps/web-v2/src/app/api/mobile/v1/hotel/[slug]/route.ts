...
/** Mobile BFF — same JSON envelope as `/api/agent/hotel/[slug]`. */
export async function GET(
  ...
  const result = await getHotelV2({ slug, locale: parsedQuery.data.locale });
  if (result === null) {
    return NextResponse.json(
  ...
  return NextResponse.json(toHotelAgentJson(result, parsedQuery.data.locale), {
